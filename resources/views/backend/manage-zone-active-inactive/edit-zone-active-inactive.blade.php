...
@push('title')
    <title>Edit Zone Active Inactive | Divyashakti Enterprise</title>
@endpush
...
@section('mainsection')
    <div class="page-wrapper">
        <div class="content container-fluid col-md-12">
            ...
            <div class="container">
                <h3 class="h3-heading mb-4">Edit Zone</h3>

                <form action="{{ url('/update-zone-active-inactive') }}/{{ $status->id }}" method="POST">
                    @csrf
                    <div class="col-lg-12 form-input-container ">
                        ...
                        <div class="mb-3  ">
                            ...
                            <input class="form-control" type="text" id="zone" placeholder="Zone" name="zone"
                                value="{{ $status->zone }}">
                            <span class="text-danger">
                                @error('zone')
                                    {{ $message }}
                        ...
                        <div class="mb-3">
                            <label for="state" class="form-label">Under Which State</label>
                            {{-- <input class="form-control" type="text" id="state" placeholder="State" name="state"
                                value="{{ old('state') }}"> --}}
                            <select name="state" id="state" class="form-select">
                                @foreach ($states as $state)
                                    <option value="{{ $state->id }}" @if ($status->state == $state->id) selected @endif>
                                        {{ $state->state }}</option>
                                @endforeach
                            </select>
                            <span class="text-danger">
