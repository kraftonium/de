...
@push('title')
    <title>Edit Area Active Inactive | Divyashakti Enterprise</title>
@endpush
...
@section('mainsection')
    <div class="page-wrapper">
        <div class="content container-fluid col-md-12">
            ...
            <div class="container">
                <h3 class="h3-heading mb-4">Edit Area</h3>

                <form action="{{ url('/update-area-active-inactive') }}/{{ $status->id }}" method="POST">
                    @csrf
                    <div class="col-lg-12 form-input-container ">
                        ...
                        <div class="mb-3  ">
                            ...
                            <input class="form-control" type="text" id="area" placeholder="Area" name="area"
                                value="{{ $status->area }}">
                            <span class="text-danger">
                                @error('area')
                                    {{ $message }}
                        ...
                        <div class="mb-3">
                            <label for="taluka" class="form-label">Under Which Taluka</label>
                            ...
                            <select name="taluka" id="taluka" class="form-select">
                                @foreach ($talukas as $taluka)
                                    <option value="{{ $taluka->id }}" @if ($status->taluka == $taluka->id) selected @endif>
                                        {{ $taluka->taluka }}</option>
                                @endforeach
                            </select>
                            <span class="text-danger">
                                @error('taluka')
